...
public class PerformanceAnalyzer {
    ...
    /**
     * Latency stats for SUCCESS messages (statusCode == 200)
     */
    private void printLatencyStatistics() {
        List<Long> latencies = records.stream()
        ...
        if (latencies.isEmpty()) {
            System.out.println("No latency data available (no successful messages)");
            return;
        }
        ...
        System.out.println("Response Time Statistics (successful messages):");
    ...
    private void printThroughputPerRoom() {
        ...
        Map<String, Long> roomCounts = records.stream()
                .collect(Collectors.groupingBy(MessageRecord::getRoomId, Collectors.counting()));
    ...
    private void printMessageTypeDistribution() {
        ...
        Map<MessageType, Long> typeCounts = records.stream()
                .collect(Collectors.groupingBy(MessageRecord::getMessageType, Collectors.counting()));
        ...
        typeCounts.entrySet().stream()
                ...
                .forEach(entry -> {
                    double percentage = (entry.getValue() * 100.0) / total;
                    System.out.println("  " + entry.getKey() + ": " +
                            entry.getValue() + " messages (" + String.format("%.2f", percentage) + "%)");
                });
    }
